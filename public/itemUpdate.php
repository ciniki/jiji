...
function ciniki_jiji_itemUpdate(&$ciniki) {
    ...
    $rc = ciniki_jiji_checkAccess($ciniki, $args['business_id'], 'ciniki.jiji.itemUpdate');
    if( $rc['stat'] != 'ok' ) {
        return $rc;
    }
    ...
    if( isset($args['title']) ) {
        ...
        if( $rc['num_rows'] > 0 ) {
            return array('stat'=>'fail', 'err'=>array('pkg'=>'ciniki', 'code'=>'3565', 'msg'=>'You already have an item with this name, please choose another.'));
        }
    }
    ...
    $rc = ciniki_core_objectUpdate($ciniki, $args['business_id'], 'ciniki.jiji.item', $args['item_id'], $args, 0x04);
    if( $rc['stat'] != 'ok' ) {
    ...
    ciniki_core_hookExec($ciniki, $args['business_id'], 'ciniki', 'web', 'indexObject', array('object'=>'ciniki.jiji.item', 'object_id'=>$args['item_id']));
